...
會看不出焦點在哪。單一分頁也有寬度上限，一個長標題不會把整格撐滿。
...
| e2e_tabs | 28 | 分頁、分割面板、拖曳分割、幾何不變式、右鍵選單、獨立監看 |
...
- **程式碼註解用英文。** 少數踩過的坑額外附一行 `中：` 摘要，因為那些是會回頭讀的。
...
已知缺口：`.txt` 檔目前被當 markdown 解析，純文字報表會被重排成一大片文字牆
（markdown 的軟換行等同空白）。純文字應該保留原始換行並用等寬字顯示。
